...
ui.note(
    f"This site is currently showing the state after gameweek {state['last_published_gameweek']} "
    f"of 2026-27, with a model trained on {state['training_rows']:,} matches. The job decides a "
    "gameweek is complete when the club that has played fewest matches has played another one, "
    "which handles postponements gracefully: a delayed match holds the count back for everybody "
    "until it is played."
)
...
with c1:
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="Gameweek %{x}<br>time factor %{y:.2f}<extra></extra>",
    ))
    fig.add_vline(x=config.GW_SIGMOID_MID, line=dict(color=theme.MUTED, width=1, dash="dash"),
