...
ui.stats([
    ...
    (f"{dummy['log_loss']:.4f}", "Baseline to beat", "A model using no information at all"),
])

ui.note(
    "**Why accuracy is the wrong headline.** A model that says 'home win, 40% confident' for "
    "every match can look accurate while being useless. What a simulation needs is "
    "well-calibrated probabilities, not confident guesses. **Log loss** measures that: it "
...
theme.apply(fig, height=320, legend=False,
            xaxis=dict(title=dict(text="Log loss (lower is better)"),
                       range=[0.95, float(comp["log_loss"].max()) * 1.07]))
ui.chart(
    fig,
    title="Log loss on the 2025-26 holdout season",
    verdict_text=(
...
with st.expander("Full comparison, including accuracy and Brier score",
    ...
    st.dataframe(
        comp_show, width="stretch", hide_index=True,
        column_config={
...
fig = go.Figure()
fig.add_trace(go.Bar(name="The model", x=labels, y=walk["log_loss"], marker_color=theme.RED,
...
with c1:
    st.markdown("##### Weight on a home win")
    ...
    theme.apply(fig, height=400, legend=False,
                xaxis=dict(title=dict(text="Model weight")))
    st.plotly_chart(fig, width="stretch")
with c2:
    st.markdown("##### Importance to XGBoost (SHAP)")
    sv = shap_vals.sort_values("mean_abs_shap")
    fig = go.Figure(go.Bar(
        y=sv["feature"], x=sv["mean_abs_shap"], orientation="h", marker_color=theme.SKY,
...
ui.verdict(
    ...
    "matters most to this project. In the linear model, <strong>the pressure metric is not "
    "statistically significant</strong> once opponent quality, home advantage and form are "
    f"included (p = {stakes_lin['p_value']:.3f}). To XGBoost it ranks <strong>{rank} of "
    "11</strong> by importance. This is left as an open disagreement rather than resolved, "
    "because the two methods measure genuinely different things: one asks whether the "
    "relationship is linear and reliable, the other how much the model leans on the feature "
    "in practice."
)
...
ui.chart(
    ...
    verdict_text=(
        ...
        f"<strong>{best['removed_feature']}</strong> costs the most to lose. Features at or "
        "below zero are earning nothing, and a couple actively improve the model by their "
        "absence, which is a normal sign that eleven features is close to as many as this much "
        "data can support."
    ),
)
...
with c1:
    st.markdown("##### When it says 70%, does it win 70% of the time?")
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=[0, 1], y=[0, 1], mode="lines", name="Perfect calibration",
        line=dict(color=theme.FAINT, width=1, dash="dash"), hoverinfo="skip",
    ))
    fig.add_trace(go.Scatter(
        ...
        marker=dict(size=calib["n_matches"] / calib["n_matches"].max() * 16 + 7, color=theme.RED),
...
with c2:
    ...
    fig = go.Figure(go.Heatmap(
        ...
        text=cm.values, texttemplate="%{text}", textfont=dict(size=16, color=theme.TEXT),
...
ui.note(
    ...
    "Several take hours, and one involved roughly 1,100 model fits. Everything else on this "
    "page is recomputed from the raw data every time the exports are rebuilt.",
    icon=":material/info:",
)
